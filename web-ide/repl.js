...
require.config({
    baseUrl: "http://try.ceylon-lang.org/scripts/modules",
    waitSeconds: 15
});
...
function complete(editor){
    ...
    jQuery.ajax('http://try.ceylon-lang.org/assist', {
...
function translate(onTranslation) {
    ...
    if (code != oldcode) {
        ...
        jquery.ajax('http://try.ceylon-lang.org/translate', {
            cache:false, type:'POST',
            dataType:'json',
            timeout:10000,
            beforeSend:startSpinner,
...
function editCode(key) {
    ...
    //Retrieve code
    jquery.ajax('http://try.ceylon-lang.org/hoverdoc?key='+key, {
        cache:true,
        dataType:'json',
        timeout:10000,
        beforeSend:startSpinner,
...
function getHoverDocs(cm) {
    ...
    document.getElementById('submit').disabled=true;
    jquery.ajax('http://try.ceylon-lang.org/hoverdoc', {
        cache:false, type:'POST',
        dataType:'json',
        timeout:10000,
        beforeSend:startSpinner,
